...
const CertificateCreator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.user_id) { setError('Silakan pilih peserta!'); return; }
  ...
  const handleReset = () => {
    ...
    setFormData({
      user_id: '', title: '', institution: '', description: '',
      issued_date: new Date().toISOString().split('T')[0]
    });
  };
  ...
  const getSelectedParticipant = () => participants.find(p => p.user_id?.toString() === formData.user_id);
      ...
      <div className="container" style={{marginTop: '-4rem', position: 'relative', zIndex: 10}}>
        <form onSubmit={handleSubmit}>
                ...
                <div className="card-body p-4">
                  <div className="mb-3">
                    ...
                    {loadingParticipants ? (
                      ...
                      <select value={formData.user_id} onChange={(e) => setFormData({...formData, user_id: e.target.value})} className="form-select" required style={{borderRadius: '10px'}}>
          ...
          <div className="pt-4 mt-2">
            <button 
              type="submit" 
              disabled={loading || !certificateImage || !selectedArea || !formData.user_id || extracting}
              className="btn btn-lg w-100 d-flex align-items-center justify-content-center gap-2 fw-semibold"
              style={{
                background: (loading || !certificateImage || !selectedArea || !formData.user_id || extracting)
                  ? '#9ca3af'
                  : `linear-gradient(150deg, ${COLORS.primary} 0%, ${COLORS.primaryDark} 100%)`,
                color: 'white',
                borderRadius: '12px',
                ...
                padding: '14px 0',
                fontSize: '1.05rem',
                boxShadow: (loading || !certificateImage || !selectedArea || !formData.user_id || extracting)
                  ? 'none'

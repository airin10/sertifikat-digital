...
def analyze_qr_payload(qr_payload: dict, qr_json: str) -> dict:
    ...
    return {
        ...
        "qr_code_analysis": {
            "version": qr_version,
            "modules": f"{qr_modules}x{qr_modules}" if isinstance(qr_modules, int) else qr_modules,
            "error_correction": "Level H (30% recovery)",
            "capacity_used_percent": round((total_bytes / max_capacity_v40) * 100, 2) if isinstance(total_bytes, int) else "N/A",
            "max_capacity_v40_bytes": max_capacity_v40,
            "status": "MUAT" if isinstance(qr_version, int) and qr_version <= 40 else "TIDAK MUAT"
        }
    }
...
async def create_certificate_single_upload(
    ...
    try:
        ...
        print(f"   • Versi QR          : {payload_analysis['qr_code_analysis']['version']}")
        print(f"   • Ukuran Modul      : {payload_analysis['qr_code_analysis']['modules']}")
        print(f"   • Error Correction  : {payload_analysis['qr_code_analysis']['error_correction']}")
        print(f"   • Kapasitas Terpakai: {payload_analysis['qr_code_analysis']['capacity_used_percent']}% dari max 1273 bytes (Versi 40)")
        print(f"   • Status            : {payload_analysis['qr_code_analysis']['status']}")
        print(f"   {'─'*60}")
        ...
        print(f"   Versi QR Code          : {payload_analysis['qr_code_analysis']['version']:>10}")
        print(f"{'='*70}\n")
